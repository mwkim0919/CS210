...
public class TranslinkService {
	...
	private final static String APIKEY = "YPcTWyKUnM4a142dF5Ys"; 
	...
	public Set<BusStop> getBusStopsAround(LatLon location, int radius) {
		...
		//http://api.translink.ca/RTTIAPI/V1/stops?lat=49.2669&long=-123.25396&radius=500&apikey=YPcTWyKUnM4a142dF5Ys
		// Make the request
		InputStream in = null;
		try {
...
public Set<BusRoute> getBusRoutes(int busStop) {
		Set<BusRoute> busRoutes = new HashSet<BusRoute>();
		// Format the request string
		StringBuilder urlBuilder = new StringBuilder(
				"//api.translink.ca/RTTIAPI/V1/");
		urlBuilder.append("routes?");
		urlBuilder.append("apiKey=" + APIKEY);
		urlBuilder.append("&stopNo=" + busStop); 
		...
		try {
			...
			busRoutes = parseBusRoutesFromJSON(s);
			...
			try {
				...
				throw new IllegalBusStopException(
						"getBusRoutes: Unable to open or read return from http request.");
			}
		}
		// Something went wrong so return an empty set
		return busRoutes;
...
private Set<BusRoute> parseBusRoutesFromJSON(String input) {
	
	// Initialize
	Set<BusRoute> routesFound = new HashSet<BusRoute>();
	JSONArray obj;
	...
	try {
		...
		if (obj != null) {
			for (int i = 0; i < obj.length(); i++) {
				...
				String routeMapLocation = route.getString("RouteMap").trim();
				String direction = route.getString("Direction").trim();
				// Deal with the routes. Parse them out and form into routes using translateRoutes
//				String stopsAsString = route.getString("Routes");
//				List<String> routesAsListOfStrings = new ArrayList<String>();
//				StringTokenizer st = new StringTokenizer(routesAsString,
//						",");
//				while (st.hasMoreTokens()) {
...
//				
				// Create the bus stop and remember it
				BusRoute busRoute = new BusRoute(routeNo);
//				addBusStop(busRoute);
				routesFound.add(busRoute);
